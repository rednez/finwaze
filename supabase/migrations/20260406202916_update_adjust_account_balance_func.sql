drop function if exists "public"."adjust_account_balance"(p_account_id bigint, p_target_balance numeric, p_local_offset interval, p_comment text);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.adjust_account_balance(p_account_id bigint, p_target_balance numeric, p_local_offset interval, p_comment text DEFAULT NULL::text, p_transacted_at timestamp with time zone DEFAULT now())
 RETURNS bigint
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$

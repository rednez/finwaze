drop function if exists "public"."make_transfer"(p_from_account_id bigint, p_to_account_id bigint, p_from_amount numeric, p_to_amount numeric, p_comment text);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.make_transfer(p_from_account_id bigint, p_to_account_id bigint, p_from_amount numeric, p_to_amount numeric DEFAULT NULL::numeric, p_local_offset interval DEFAULT '00:00:00'::interval, p_comment text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
BEGIN
  ...
  INSERT INTO public.transactions (account_id, transaction_amount, charged_amount, type, category_id, transfer_id, comment, transaction_currency_id, local_offset)
  VALUES (p_from_account_id, -p_from_amount, -p_from_amount, 'transfer', internal_category_id, tid, p_comment, from_currency_id, p_local_offset);

  INSERT INTO public.transactions (account_id, transaction_amount, charged_amount, type, category_id, transfer_id, comment, transaction_currency_id, local_offset)
  VALUES (p_to_account_id, actual_to_amount, actual_to_amount, 'transfer', internal_category_id, tid, p_comment, to_currency_id, p_local_offset);
END;
$function$
;
